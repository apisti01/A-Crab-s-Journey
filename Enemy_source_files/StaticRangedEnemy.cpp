...
StaticRangedEnemy::StaticRangedEnemy(std::string name, const sf::Texture &texture, Collider collider,
                                     std::unique_ptr<Weapon> weapon, float hp, float maxHp, float armor, float maxArmor,
                                     float strength, float maxStrength, float XpReward,
                                     int coinsDropped, int pearlsDropped) :
        Enemy(std::move(name), texture, std::move(collider), std::move(weapon), hp, maxHp,
              0, 0, armor, maxArmor, strength, maxStrength, XpReward, coinsDropped, pearlsDropped) {
